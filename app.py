...
@app.route('/api/campaigns/urls', methods=['POST', 'OPTIONS'])
def get_campaign_urls():
    """
    Obtiene las URLs finales únicas utilizadas en los anuncios de una campaña.
    Útil para sugerir URLs al usuario.
    """
    # CORS preflight
    if request.method == 'OPTIONS':
        response = jsonify({'status': 'ok'})
    ...
    try:
        ...
        if not all([customer_id, campaign_id]):
            return jsonify({
                "success": False,
                "message": "Faltan customerId o campaignId"
            }), 400
        ...
        # Query para obtener URLs de anuncios
        query = f"""
        ...
        print(f"❌ Error obteniendo URLs de campaña: {str(ex)}")
        result = jsonify({
            "success": False,
            "message": str(ex),
            "urls": []
